...
# 读入数据
brain_8gene = pd.read_csv(f"D:/Desktop/BCBM_by_ML/file_input/brain_8gene.csv")
metadata = pd.read_csv(f"D:/Desktop/BCBM_by_ML/file_input/sample_metadata.csv")
...
def get_TpTN_FpFn(list1, list2):
    ...
    for i in range(len(reallable_list)):
        if reallable_list[i] == 0 and predictlable_list[i] == 0:
            TP_count += 1
        if reallable_list[i] == 1 and predictlable_list[i] == 1:
            TN_count += 1
        if reallable_list[i] == 1 and predictlable_list[i] == 0:
            FP_count += 1
        if reallable_list[i] == 0 and predictlable_list[i] == 1:
            FN_count += 1
