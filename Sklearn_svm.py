"""
使用sklearn，svm，留一法对样本进行二分类
"""

import pandas as pd
import numpy as np
from sklearn.svm import SVC # SVC是分类，SVR是回归
...
# 读入数据
brain_8gene = pd.read_csv(f"D:/Desktop/BCBM_by_ML/file_input/brain_73_8gene_test.csv")
metadata = pd.read_csv(f"D:/Desktop/BCBM_by_ML/file_input/sample_metadata_73_test.csv")
...
### -----------------------------------------------------------------------------
# 计算F1_score, Accuracy, Sensitivity, Specificity
### -----------------------------------------------------------------------------
...
#  计算AUC，绘制ROC
### ---------------------------------------------------------------------------
real = np.array(list(flatten(reallable_list)))
roc_auc = dict()
